...
// bool isDotEnabled = 0;
// bool isTimeTicking = 1;
// bool isSettingsOpened = 0;
// bool isParameterOpened = 0;
// bool isResetMenuOpened = 0;
uint8_t displayShows = 0;
uint8_t choosedOption;
bool stepOnTimeSetup = 0;
uint8_t nowShow = 0;
uint8_t times[3] = {500, 1000, 100};
unsigned long timer;
String options[] = {"Time Setting", "Date Setting", "Brightness", "Reset", "Exit"};
  ...
  if (displayShows == 0) {
    if (leftBtn.hold()) {
      ...
      if (!settings.quietMode) {
        tone(BUZZIER_PIN, 1500, 1000);
      }
      EEPROM.put(0, settings);
      bool workingFunc = show_quiet_mode(settings.quietMode, BUZZIER_PIN);
      while (!workingFunc) {}
      // isTimeTicking = true;
      displayShows = 0;
    }
    ...
    if (centerBtn.hold()) {
      ...
      choosedOption = 0;
      if (!settings.quietMode)
        tone(BUZZIER_PIN, 1500, 500);
      show_settings(choosedOption, options, options_max);
    }

    if (rightBtn.click()) {
      if (nowShow < sizeof(times) - 1) {
        clear();
        nowShow++;
        timer = times[nowShow];
        if (!settings.quietMode)
...
uint8_t get_weekday_from_date(uint8_t day, uint8_t month, uint8_t year) {
  // uint8_t codes_months[12] = {6, 2, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4};
...
void setup() {
  ...
  // watch.settime(0,30,12,1,1,24,1);
}
